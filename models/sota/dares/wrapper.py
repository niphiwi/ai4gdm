...
class Dares:
    def __init__(self):
        ...
        laplace_path = os.path.join(current_dir, "mesh", "laplace.npz")
        grad_x_path = os.path.join(current_dir, "mesh", "grad_x.npz")
        grad_y_path = os.path.join(current_dir, "mesh", "grad_y.npz")
        verts_path = os.path.join(current_dir, "mesh", "vertices.npy")
    ...
    def predict(self):
        ...
        grid_x_regular, grid_y_regular = np.meshgrid(np.linspace(self.x_min, self.x_max, 64), np.linspace(self.y_min, self.y_max, 64))
